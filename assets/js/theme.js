...
class Theme {
    ...
    const autosearch = autocomplete(inputId, {
      ...
      source: (query, callback) => {
        ...
        if (searchConfig.type === 'lunr') {
          ...
          fetch(searchConfig.searchUrl, {
            ...
            if (!response.ok) {
              throw new Error("Websearch API request failed with status ".concat(response.status));
            }
      ...
      templates: {
        suggestion: _ref5 => {
          ...
          return "<div>\n                                        <span class=\"suggestion-title\">".concat(title, "</span>\n                                        <span class=\"suggestion-date\">").concat(date, "</span>\n                                    </div>\n                                    <div class=\"suggestion-context\">").concat(context, "</div>\n                                    ");
        },
        empty: _ref6 => {
          let {
            query
  ...
  initSearch() {
    ...
    const searchInputTop = "#search-input-".concat(suffix);
    const searchDropdownTop = "#search-dropdown-".concat(suffix);
    ...
    if ($searchInputHero) {
      ...
      $searchInputHero.addEventListener('input', () => {
        if ($searchInputHero.value === '') $searchClearHero.style.display = 'none';else $searchClearHero.style.display = 'inline';
      }, false);
  ...
  initTypeit() {
    if (this.config.typeit) {
      ...
      Object.values(typeitConfig.data).forEach(group => {
        const typeone = i => {
          const id = group[i];
          const instance = new TypeIt("#".concat(id), {
